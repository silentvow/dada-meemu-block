    ...
    switch (req.method) {
      case 'GET': {
        onValue(scoresRef, snapshot => {
          ...
          res.status(200).json({
            data: {
              story: formatData(data.story || DEFAULT_RECORDS),
              extra_story: formatData(data.extra_story || DEFAULT_RECORDS),
              challenge_dada: formatData(data.challenge_dada || DEFAULT_RECORDS),
              challenge_yoda: formatData(data.challenge_yoda || DEFAULT_RECORDS),
